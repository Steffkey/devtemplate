...
cat("This deviation protocol is based on the PRP-DEV deviation template (version 1).", "\n\n", sep ="")
...
for (i in 5:length(params$answers)) # titelseite wird separat formatiert, danach loop durch alle items
{
  if(grepl("hxxd_", params$questions[[i]])) # find the headings (marked with "hxxd_")
  {
    ...
    else if(!any(grepl("Uncategorized/Other", params$answers$D1))) # we need a workaround for the last section
